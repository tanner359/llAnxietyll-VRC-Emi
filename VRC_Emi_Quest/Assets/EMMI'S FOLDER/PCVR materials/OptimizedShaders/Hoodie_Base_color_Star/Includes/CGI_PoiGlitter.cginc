    ...
    void applyGlitter(inout float4 albedo, inout float3 glitterEmission)
    {
        ...
        switch(float(0))
        {
            ...
            finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.17)), poiCam.viewDir)), float(121)), 0);
        ...
        glitterColor *= lerp(1, albedo, float(0.132));
        #if defined(PROP_GLITTERCOLORMAP) || !defined(OPTIMIZER_ENABLED)
        ...
        if(float(0) == 1)
        {
            albedo.rgb = lerp(albedo.rgb, finalGlitter * glitterColor * float(1.6), finalGlitter * glitterTexture.a * glitterMask);
            glitterEmission = finalGlitter * glitterColor * max(0, (float(1.6) - 1) * glitterTexture.a) * glitterMask;
        }
        else
        {
            glitterEmission = finalGlitter * glitterColor * float(1.6) * glitterTexture.a * glitterMask;
        }
    }
#endif
