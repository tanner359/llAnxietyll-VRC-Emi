...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    float rimWidth = lerp( - .05, 1, float(0.519));
    #ifdef POI_AUDIOLINK
    ...
    rim = 1 - smoothstep(min(float(0.142), rimWidth), rimWidth, viewDotNormal);
    rim *= float4(0,0,0,1).a * rimColor.a * rimMask;
    rimLightEmission = rim * lerp(albedo, rimColor, float(0.129)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, float(0.129)) + lerp(albedo.rgb, rimColor, float(0.129)) * rimBrighten, rim);
}
#endif
