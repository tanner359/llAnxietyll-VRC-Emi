...
{
    float3 emission0 = 0;
    float emissionStrength0 = float(7.46);
    ...
    if (float(1))
    {
        ...
        {
            #if defined(PROP_EMISSIONSCROLLINGCURVE) || !defined(OPTIMIZER_ENABLED)
                emissionStrength0 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve, _MainTex, TRANSFORM_TEX(poiMesh.uv[float(0)], _EmissionScrollingCurve) + (dot(pos, float4(0,-10,0,0).xyz) * float(30)) + _Time.x * float(3)).r;
            #endif
        }
        else
        {
            emissionStrength0 *= calculateScrollingEmission(float4(0,-10,0,0).xyz, float(3), float(30), float(10), float(0), pos);
        }
    }
    
    if (float(0))
    {
        emissionStrength0 *= calculateBlinkingEmission(float(0), float(1), float(1), float(0));
    }
