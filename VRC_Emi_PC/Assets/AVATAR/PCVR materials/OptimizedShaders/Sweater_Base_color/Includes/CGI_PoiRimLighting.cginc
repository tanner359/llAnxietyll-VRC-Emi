...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    #endif
    #if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
        rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * float4(0.005536096,0.005536096,0.005536096,1);
    #else
        rimColor = float4(0.005536096,0.005536096,0.005536096,1);
    #endif
    ...
    rim *= float4(0.005536096,0.005536096,0.005536096,1).a * rimColor.a * rimMask;
    rimLightEmission = rim * lerp(albedo, rimColor, float(0.5)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, float(0.5)) + lerp(albedo.rgb, rimColor, float(0.5)) * rimBrighten, rim);
}
#endif
